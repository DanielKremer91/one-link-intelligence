...
def normalize_url(u: str, remove_www: bool = False) -> str:
    try:
        ...
        # Sort params for canonical equality
        ...
        rebuilt = urlunparse((p.scheme, hostname + (":" + str(p.port) if p.port else ""), path, "", query, fragment))
        return rebuilt
    except Exception:
...
def is_content_position(position_raw) -> bool:
    ...
    return any(token in pos_norm for token in ["inhalt", "content", "body", "main", "artikel", "article"])  # extendable
...
with st.sidebar:
    ...
    not_similar_threshold = st.slider("Unähnlichkeits-Schwelle (zweite Analyse) – Links ≤ Wert gelten als schwach", 0.0, 1.0, 0.60, 0.01)
...
mode = st.radio("Eingabemodus", ["Ein Excel mit Tabs", "Einzeltabellen hochladen", "URLs + Embeddings"], horizontal=True)

related_df = inlinks_df = metrics_df = backlinks_df = None
emb_df = None

if mode == "Ein Excel mit Tabs":
    xls = st.file_uploader("Excel hochladen (enthält Tabs: Related URLs, All Inlinks, Linkmetriken, Backlinks)", type=["xlsx", "xlsm", "xls"])
    if xls is not None:
        try:
            xls_obj = pd.ExcelFile(xls)
            # Try flexible tab name matching
            ...
            tab_inlinks = get_sheet_like(["All Inlinks", "Inlinks", "all inlinks"]) or xls_obj.sheet_names[1 if len(xls_obj.sheet_names) > 1 else 0]
            tab_metrics = get_sheet_like(["Linkmetriken", "Metrics", "linkmetriken"]) or xls_obj.sheet_names[2 if len(xls_obj.sheet_names) > 2 else 0]
            tab_backlinks = get_sheet_like(["Backlinks", "Ref Domains", "backlinks"]) or xls_obj.sheet_names[3 if len(xls_obj.sheet_names) > 3 else 0]
    ...
    related_df = read_any(related_up)
    inlinks_df = read_any(inlinks_up)
    metrics_df = read_any(metrics_up)
    backlinks_df = read_any(backlinks_up)
elif mode == "URLs + Embeddings":
    st.write("Lade eine Tabelle mit **URL** und **Embedding** (JSON-Array oder durch Komma/Leerzeichen getrennte Zahlen). Zusätzlich werden **All Inlinks**, **Linkmetriken** und **Backlinks** benötigt.")
    ...
    if emb_df is not None:
        ...
        for c in cols:
            ...
            if url_col is None and lc in ("url", "urls", "page", "seite"):
                url_col = c
...
", " ").replace("	", " ").replace(";", " ").replace("|", " ").split(" ") if p]
        ...
        if len(vecs) == 0:
            st.error("Keine gültigen Embeddings erkannt.")
        else:
            # Pad/trim to consistent dimensionality
            ...
            # Top-k Nachbarn je URL (ohne Selbsttreffer), threshold = sim_threshold
            K = int(max_related)
            pairs = []  # (target, source, sim)

            if backend == "Schnell (FAISS)" and 'faiss_available' in globals() and faiss_available:
                import faiss  # type: ignore
                dim = V.shape[1]
                index = faiss.IndexFlatIP(dim)  # Inner Product == Cosine bei L2-normalisierten Vektoren
                Vf = V.astype('float32')
                index.add(Vf)
                # +1, weil der Self-Match typischerweise auf Rang 1 landet
                topk = min(K + 1, V.shape[0])
                ...
                for i in range(V.shape[0]):
                    sims = V[i] @ V.T  # 1 x N
                    sims[i] = -1.0  # Self-Match ausschließen
                    # Kandidaten holen
                    if K < V.shape[0] - 1:
            ...
            if len(pairs) == 0:
                st.warning("Keine Related-URLs über der Ähnlichkeitsschwelle gefunden.")
            related_df = pd.DataFrame(pairs, columns=["Ziel", "Quelle", "Similarity"])  # wie GAS erwartet
...
# Expect columns: [url, score, inlinks, outlinks] in any naming; we'll assume first 4 cols as in Apps Script
...
for i in range(1, int(max_related) + 1):
    cols.extend([f"Related URL {i}", f"überhaupt verlinkt {i}?", f"aus Inhalt heraus verlinkt {i}?", f"Linkpotenzial {i}"])
...
for target, related_list in sorted(related_map.items()):
    ...
    for source, sim in related_sorted:
        ...
        row.extend([source, anywhere, from_content, final_score if show_numbers_in_heatmap else final_score])
...
st.download_button("Download 'Interne Verlinkungsmöglichkeiten' (CSV)", data=csv1, file_name="interne_verlinkungsmoeglichkeiten.csv", mime="text/csv")
...
# Build similarity map for both directions (no threshold for inclusion here; we'll compare to slider threshold)
sim_map: Dict[str, float] = {}
processed_pairs2 = set()

for _, r in related_df.iterrows():
    a = normalize_url(r.iloc[1], remove_www)  # Quelle in deinem GAS-Code
    b = normalize_url(r.iloc[0], remove_www)  # Ziel in deinem GAS-Code
    try:
        sim = float(str(r.iloc[2]).replace(",", "."))
    except Exception:
        continue
...
# PageRank Waster-like raw score and backlink-adjusted score
...
for u, raw in raw_score_map.items():
    ...
    malus = 5.0 * (2.0 if backlink_weight_2x else 1.0) if impact == 0 else 0.0
    adjusted = (raw or 0.0) - ((2.0 if backlink_weight_2x else 1.0) * impact) + malus
...
# Display with background color in a separate column (Streamlit can't natively color rows without Styler)
out_df["Farbcode (intern)"] = colors
st.dataframe(out_df, use_container_width=True, hide_index=True)

csv2 = out_df.to_csv(index=False).encode("utf-8")
st.download_button("Download 'Potenziell zu entfernende Links' (CSV)", data=csv2, file_name="potenziell_zu_entfernende_links.csv", mime="text/csv")
